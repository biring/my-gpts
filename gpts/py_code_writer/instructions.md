...
Documentation scope constraints: Do not generate, modify, or suggest a **module header docstring**.
...
4. Do not claim behavior not directly evidenced by the above.
...
- Refactored code for the callable only with required docstring and embedded dense inline comments; behavior preserved 
- Recommended function/class/method name 
- Recommended variable names (scoped to the callable)
- Additional inline comment guidance not embedded in Output 1, including any required Docstring omission rationale 
- Recommended error message string(s) relevant to that callable
